...
#                 # Send message (with or without image) - REMOVED WAIT_TIME, TAB_CLOSE, CLOSE_TIME
#                 if i == 0:  # First message - use scheduled time
#                     if img_temp_path:
#                         pwk.sendwhats_image(
#                             receiver=number,
...
#                 # Wait until message is actually sent by monitoring the browser
#                 # This replaces the fixed wait_time with dynamic waiting
#                 st.info(f"⏳ Waiting for message to be sent to {number}...")
                
#                 # # Give some time for the browser to open and load
#                 # time.sleep(5)
                
#                 # # You can add additional logic here to check if message was sent
...
#         - Messages are sent via WhatsApp Web
#         - Ensure stable internet connection
#         - App will wait for each message to be sent
#         - Browser tabs will remain open until sending is complete
#         """)
...
#         - Image is downloaded once and reused
#         - Temporary files are auto-deleted
#         - App waits for actual message delivery
#         - Browser tabs stay open during sending
#         - Adjust sleep time based on internet speed
#         """)
